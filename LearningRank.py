...
for low in range(2263, 4469,46):
    excel_file = '2ランキング.xlsx'
    workbook = openpyxl.load_workbook(excel_file)
    sheet = workbook.active
    # print(low)

        # データの処理と挿入
    for row in range(low+4, low+45, 2):
        # print(row)
        ...
        this_number = mojimoji.zen_to_han(sheet['B'+str(low+1)].value,kana=False)
    ...
    with open('楽曲データ2.csv', 'w', newline='',encoding='UTF-8') as f:
        # CSVライターオブジェクトを作成
